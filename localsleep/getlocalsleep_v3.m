...
%% 参数设置
input_dir = 'D:\桌面\T0_ANT数据v2\ICA后未分段全部数据\ICA2分段后数据'; % .set文件存储路径
output_dir = 'D:\桌面\T0_ANT数据v2'; % 结果输出路径
...
for subj_idx = 1:n_subjects
    try
        ...
        for epoch_idx = 1:EEG.trials
            ...
            %% 慢波检测
            [twa_results] = twalldetectnew_TA_v2(temp_data, EEG.srate, 10);
            % 6. 结果保存
            ...
            all_durations = [all_durations; twa_results.all_wave_durations(:)]; % 收集所有波
            epoch_count = epoch_count + 1;
        end
        
        subject_param_means = subject_param_means / count;
        global_sw_density = global_sw_density / epoch_count;
        mean_duration = mean(all_durations, 'omitnan');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % 新增：累计每个慢波的持续时间
    wave_duration = (wavend - wavest) * (orig_fs/fs) / orig_fs;  % 校正采样率差异
    if ~isfield(twa_results, 'all_wave_durations')
        twa_results.all_wave_durations = wave_duration;
    else
        twa_results.all_wave_durations(end+1) = wave_duration;
    end
     wvi=wvi+1;
...
if num_channels > 0 && total_time_global > 0
    % 平均慢波密度(波/秒/通道)
    twa_results.global_sw_density = twa_results.global_sw_count / total_time_global;

    % 平均慢波持续时间：使用所有慢波的持续时间求平均
    if isfield(twa_results, 'all_wave_durations') && ~isempty(twa_results.all_wave_durations)
        twa_results.mean_duration = mean(twa_results.all_wave_durations, 'omitnan');
    else
        twa_results.mean_duration = 0;
    end
else
